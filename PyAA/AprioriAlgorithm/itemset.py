...
def itemsets_from_transactions(
    ...
    large_itemsets: typing.Dict[int, typing.Dict[tuple, int]] = {
        1: {item: count for (item, count) in candidates.items() if (count / len(manager)) >= 2/transaction_count}
    }
    ...
    while large_itemsets[k - 1] and (max_length != 1):
        ...
        '''This is also where my code comes in!!!!'''
        #min_support = findminsupport(C_k, transaction_count, k)
